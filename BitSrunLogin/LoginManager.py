...
class LoginManager:
	...
	def _get_login_page(self):
		...
		req = request.Request(self.url_login_page)
		req.add_header('User-Agent', header['User-Agent'])
		try:
			with request.urlopen(req, timeout=0.1) as f:
				self._page_response = f.read().decode('utf-8')
			# print(self._page_response)
		except Exception as e:
			print("获取登录页面失败，发送GET请求失败！！！错误：%s，URL："%e, self.url_login_page)
			self._page_response = "id=\"user_ip\" value=\"127.0.0.1\">"
	...
	def _resolve_ip_from_login_page(self):
		self.ip = re.search("id=\"user_ip\" value=\"([0-9\.]+?)\">", self._page_response).group(1)
	...
	def _get_challenge(self):
		...
		data = bytes(parse.urlencode(params_get_challenge), encoding = 'utf-8')
		req = request.Request(self.url_get_challenge_api, data=data, headers=header)
		try:
			with request.urlopen(req, timeout=0.1) as f:
				self._challenge_response = f.read().decode('utf-8')
			# print(self._page_response)
		except Exception as e:
			self._challenge_response = '"challenge":"123456"'
			print('获取登录页面失败，发送GET请求失败！！！错误：%s，URL：'%e, self.url_get_challenge_api)
	

		# self._challenge_response = requests.get(, params=params_get_challenge, headers=header)
	...
	def _resolve_token_from_challenge_response(self):
		self.token = re.search('"challenge":"(.*?)"', self._challenge_response).group(1)
	...
	def _send_login_info(self):
		...
		data = bytes(parse.urlencode(login_info_params), encoding = 'utf-8')
		req = request.Request(self.url_login_api, data=data, headers=header)
		try:
			with request.urlopen(req, timeout=0.1) as f:
				self._login_responce = f.read().decode('utf-8')
		except Exception as e:
			print('获取登录页面失败，发送GET请求失败！！！错误：%s，URL：'%e, self.url_login_api)
			self._login_responce = '{"res":"error", "error_msg":"get request Failed"}'
	...
	def _resolve_login_responce(self):
		result = json.loads(re.findall('{.+?}', self._login_responce)[0])
		if result['res'] == 'ok':
			self._login_result = result['suc_msg']
		else:
			self._login_result = result['error_msg']
